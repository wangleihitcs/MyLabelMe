...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None, output=None, store_data=True):
        ...
        # ************************************************************************************************
        S = action(text='&Last', slot=self.S,
                   shortcut='Ctrl+S', icon='undo', tip='Quit application')
        X = action(text='&Next', slot=self.X,
                   shortcut='Ctrl+X', icon='undo', tip='Quit application')

        self.tools = self.toolbar('Tools')
        self.actions.beginner = (
            open, save, None, create, copy, delete, None,
            zoomIn, zoom, zoomOut, fitWindow, fitWidth, None, S, X)

        self.actions.advanced = (
            open, save, None,
            createMode, editMode, None,
            hideAll, showAll, None, S, X)
        # ************************************************************************************************
    ...
    def newShapeXX(self):
        ...
        """
        print(self.canvas.shapes.__len__())
    ...
    ################################################################################### origin
    def openFileXX(self, _value=False):
        if not self.mayContinue():
            return
    ...
    #################################################################################### origin
    def saveFileXX(self, _value=False):
    ...
    def openFile(self, _value=False):
        ...
        formats = ['*.{}'.format(fmt.data().decode())
                   for fmt in QImageReader.supportedImageFormats()]
        filters = QFileDialog.ShowDirsOnly
        ...
        print('*'*40 + 'all png read success!')
        self.filename_list = sorted(filename_list)

        self.all_data_list = []
        for file_name in self.filename_list:
            data = {}
            data['id'] = os.path.join(dirpath, file_name)
            # print(data['id'])
            data['shapes'] = []
            data['filename'] = file_name.split('.')[0]
        ...
        #     self.loadFile(filename)
        self.index = 0
        # print(self.all_data_list[self.index]['id'])
        # for data in self.all_data_list:
        #     print(data['id'])
        self.loadFile(self.all_data_list[self.index]['id'])
    ...
    def newShape(self):
        ...
        if text is not None:
            ...
            # print(shape)
            self.all_data_list[self.index]['shapes'].append(shape)
            # print(self.all_data_list[self.index]['shapes'][-1].fill_color)
            self.addLabel(shape)
            if self.beginner(): # Switch to edit mode.
                self.canvas.setEditing(True)
                self.actions.create.setEnabled(True)
    ...
    def S(self):
        if self.index >= 1:
            ...
            # print(self.all_data_list[self.index]['id'])
            self.loadFile(self.all_data_list[self.index]['id'])
            shapes = self.all_data_list[self.index]['shapes']
            for shape in shapes:
                self.addLabel(shape)
            self.canvas.loadShapes(shapes)
        else:
    ...
    # next image
    def X(self):
        if self.index < self.all_data_list.__len__():
            self.index += 1
            # print(self.index)
            # print(self.all_data_list[self.index]['id'])
            self.loadFile(self.all_data_list[self.index]['id'])
